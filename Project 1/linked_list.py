...
class Node:
    ...
    def __repr__(self):
        return 'Node(%d, %s)' % (self.val, repr(self.next))

    def __eq__(self, other):
        return isinstance(other, Node) and other.val == self.val and other.next == self.next
...
def insert(lst, val, pos):
    ...
    return Node(lst.val, insert(lst.next, val, pos - 1))
...
def pop(lst, pos):
    ...
    temp_1, temp_2 = pop(lst.next, pos - 1)
...
def reverse(lst):
    ...
    Args:
        lst(Node): the head of a LinkedList
    Returns:
         lst(Node): the reversed LinkedList
    """
    if lst is None:
        return None
    head, tail = helper
    return reverse(lst.next)